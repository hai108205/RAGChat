...
const totalTokensUsedInLifetime = asyncHandler(async (req: Request, res: Response) => {
...
const tokensUsedByGroup = asyncHandler(async (req: Request, res: Response) => {
    const groupBy = (req.params.groupBy as string) || "";
    const { from, to } = req.query;

    validateGroupBy(groupBy);

    if (from && isNaN(Date.parse(from as string))) {
        const err: any = new Error("Invalid 'from' date");
        err.status = 400;
        err.statusCode = 400;
        throw err;
    }
    ...
    if (to && isNaN(Date.parse(to as string))) {
        const err: any = new Error("Invalid 'to' date");
        err.status = 400;
        err.statusCode = 400;
        throw err;
    }
    ...
    switch (groupBy) {
        ...
        case "month":
            ...
            break;
        default: {
            const err: any = new Error("Invalid groupBy");
            err.status = 400;
            err.statusCode = 400;
            throw err;
        }
    ...
    const usageByGroup = await prisma.$queryRaw<Array<{
        period: Date;
        model: string;
        provider: string;
        totalInput: number | bigint;
        totalOutput: number | bigint;
    ...
    const serializedUsage = (usageByGroup || []).reduce((acc: Record<string, any>, curr) => {
...
const topChatsByTokensUsed = asyncHandler(async (req: Request, res: Response) => {
    ...
    const chatIds = topChats.map((u) => u.chatId).filter(Boolean) as string[];
...
const usageBreakdownByModel = asyncHandler(async (req: Request, res: Response) => {
    const { from, to, page = 1, limit = 10 } = req.query;
    const offset = (Number(page) - 1) * Number(limit);

    const conditions = [Prisma.sql`u."user_id" = ${req.user.id}`];
    if (from) conditions.push(Prisma.sql`u."timestamp" >= ${new Date(from as string)}`);
    if (to) conditions.push(Prisma.sql`u."timestamp" <= ${new Date(to as string)}`);
